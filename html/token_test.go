...
func TestDuplicateAttributesNotRendered(t *testing.T) {
	// A duplicate attribute must not survive a Parse/Render round trip: a
	// caller that inspects the first href cannot be made to re-emit a second,
	// unchecked one for the browser to pick up instead.
	...
	if got := strings.Count(buf.String(), `href=`); got != 1 {
		t.Errorf("got %d href attributes, want 1: %s", got, buf.String())
	}
	if strings.Contains(buf.String(), "javascript:") {
		t.Errorf("duplicate attribute value survived rendering: %s", buf.String())
	}
}
